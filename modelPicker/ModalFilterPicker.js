...
export default class ModalFilterPicker extends Component {
  constructor (props, ctx) {
    ...
    this.state = {
      ...
      ds: new ListView.DataSource({
        rowHasChanged: (r1, r2) => r1.clubid !== r2.clubid
  ...
  renderOption = (rowData) => {
    ...
    //const { key, label } = rowData; // change here for filter
    const { clubid, clubname } = rowData; // change here for filter


    let style = styles.optionStyle
    let textStyle = optionTextStyle||styles.optionTextStyle

    if (clubid === selectedOption) {
      style = styles.selectedOptionStyle
      textStyle = selectedOptionTextStyle ||styles.selectedOptionTextStyle
    }

    if (renderOption) {
      return renderOption(rowData, clubid === selectedOption)
    } else {
      return (
        <TouchableOpacity activeOpacity={0.7}
          style={style}
          onPress={() => this.props.onSelect(clubid)}
        >
          <Text style={fontSize=14}>{clubname +" | "+ rowData.location +" | "+rowData.city}</Text>
        </TouchableOpacity>
      )
    }
  }
  ...
  onFilterChange = (text) => {
    ...
    const filtered = (!filter.length)
      ? options
      : options.filter(({ searchKey, clubname, key }) => (
        0 <= clubname.toLowerCase().indexOf(filter) ||
